...
def get_similar_users(db: Session, user_id: int):
    similar_users = (
        db.query(Similarity)
        .filter(Similarity.user_id == user_id, Similarity.similarity_score > 0.1)
        .order_by(Similarity.similarity_score.desc())
        .all()
    )
    result = []
    seen_users = set()
    for similar_user in similar_users:
        if similar_user.similar_user_id not in seen_users:
            profile = (
                db.query(Profile)
                .filter(Profile.user_id == similar_user.similar_user_id)
                .first()
            )
            if profile:
                interests = (
                    ...
                    .filter(
                        user_interest_association.c.user_id
                ...
                result.append(
                    {
                        ...
                        "similarity_score": math.ceil(
                            similar_user.similarity_score * 100
                        ),
                        "interests": interest_names,
                        "is_active": similar_user.is_active,
                    }
                )
                seen_users.add(similar_user.similar_user_id)
    return result